  ...
  return nowPlayingMovies.length > 0 ? (
    <>
            ...
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
              ...
              {selectedMovie === movie.id && (
                <div className='absolute top-2 ring-2 flex items-center justify-center bg-primary h-6 w-6 rounded '>
                  <CheckIcon className='w-4 h-4 to-white' strokeWidth={2.5} />
                </div>
              )}
      ...
      {Object.keys(dateTimeSelection).length > 0 && (
        <div className='mt-6'>
          <h2 className='mb-2'>Selected Dtae-Time</h2>
            ...
            {Object.entries(dateTimeSelection).map(([date, times]) => (
              <li key={date}>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                  {times.map((time) => (
                    <div key={time} className='border border-primary px-2 py-1 flex items-center roudned'>
                      <span>{time}</span>
